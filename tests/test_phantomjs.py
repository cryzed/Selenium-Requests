...
def test_headers():
    ...
    # These are the default headers sent for the PhantomJS browser, it's easier
    # to simply check that the values are not empty instead of comparing them
    # to constants, since those would change frequently with each iteration of
    # the used browser. Additionally the existence of headers such as
    # Accept-Language and Referer confirms that these are not simply the
